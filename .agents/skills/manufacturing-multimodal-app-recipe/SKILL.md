...
2. Ask **Question 0 (mode)** first. If **demo**, branch to
   ...
   [`references/DEMO_POC.md`](references/DEMO_POC.md), skip questions 1–9; else
   (**production**, default) continue.
3. Ask the 9 questions in ONE batched message (defaults in brackets); accept
   `go`/`defaults`/empty. Question 9 selects the **agentic explainability**
   path.
...
| `{{STACK_DIR}}` | e.g. `weld-defect-stack`, `cnc-tool-wear-stack`, `pcb-inspection-stack` |
...
[Demo/PoC mode](#demopoc-mode); skip questions 1–9 (they apply to
`production` only).

1. Vision model [`{{DEFAULT_MODEL}}`] (also: `{{OTHER_MODELS}}`) — classifier or detector
2. Vision device [CPU] (GPU, NPU, AUTO)
3. Video input [simulator sample video] (or RTSP URL / `/dev/videoN` / local path); sets DLSPS source
4. Sensor pattern [pretrained model] (threshold, rate-of-change, rolling z-score,
   pretrained model — see `time-series-analytics-user` `references/patterns.md`)
5. Sensor input [simulator sample CSV] (or MQTT/OPC UA live feed); sets Telegraf `TELEGRAF_INPUT_PLUGIN`
6. Fusion mode [`{{FUSION_MODE}}`, default `OR`] + `{{TOLERANCE_NS}}` [default `50e6`]
7. Alert channels [MQTT `{{SENSOR_ALERT_TOPIC}}` + `{{FUSION_TOPIC}}`]
8. Dashboard slug [`{{DASHBOARD_SLUG}}`]
9. LLM/VLM explainability layer over fused alerts? [`{{AGENTIC}}`, default `no`]
...
failure. The script body and full **validation rules table** (`MODE`,
`HOST_IP`, `FUSION_MODE`∈`AND`/`OR`, `TOLERANCE_NS` numeric, `PIPELINE_NAME`,
topics, TURN creds, inputs, Agentic params, …) are in
...
When Question 9 selects Agentic, **add** an LLM/VLM layer that subscribes to
