...
3. Ask the 8 questions in ONE batched message (defaults in brackets); accept
   `go`/`defaults`/empty.
...
| `{{STACK_DIR}}` | top-level checkout/working dir if generating a standalone copy rather than adding to an existing clone (default: work in place inside `industrial-edge-insights-time-series/`) |
...
`time-series-analytics-user` directly — skip questions 1–8.

1. App name [`{{APP_NAME}}`] — new `apps/<name>/` folder to create
2. Sensor pattern [pretrained model] (threshold, rate-of-change, rolling
   z-score, pretrained model — see `time-series-analytics-user`
   `references/patterns.md`) + `{{SENSOR_TAGS}}`
3. Ingestion transport [`opcua`] (or `mqtt`) — drives which simulator/Telegraf
   input plugin is used
4. Real device or simulator? [simulator with a sample CSV] (or a real
   OPC-UA server URL / MQTT broker to connect Telegraf to directly)
5. Alert channel [`mqtt`, `{{ALERT_TOPIC}}`] (or `opcua`) — **enable only one**
6. Windowed batch inference needed? [`no`] (`yes` ships `config-batch.json`
   in addition to the streaming `config.json`)
7. Dashboard title [`{{DASHBOARD_TITLE}}`]
8. Multi-stream benchmarking? [`{{NUM_STREAMS}}=1`] (`>1` load-tests with
